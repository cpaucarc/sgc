        ...
        <x-utils.tables.table>
            ...
            @slot('body')
                @foreach($facultades as $facultad)
                    <x-utils.tables.row>
                        <x-utils.tables.body class="font-bold">{{$facultad->nombre}}</x-utils.tables.body>
                        <x-utils.tables.body>{{$facultad->abrev}}</x-utils.tables.body>
                        <x-utils.tables.body>{{$facultad->direccion}}</x-utils.tables.body>
                    </x-utils.tables.row>
                @endforeach
            @endslot
        </x-utils.tables.table>
    @else
        <div class="border border-gray-300 rounded-md">
            <x-utils.message-no-items
                title="Aún no hay facultades registrados">
                @slot('icon')
