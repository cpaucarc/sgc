<div>
    <x-jet-dialog-modal wire:model="open">
            ...
            <div class="space-y-4 divide-y divide-dashed divide-gray-200">
                ...
                <div class="flex justify-between items-center">
                    <p class="font-light text-sm text-gray-600">
                        @if(count($selected))
                            {{ count($selected) }} {{ count($selected) == 1 ? 'entrada seleccionado' : 'entradas seleccionados' }}
                        @else
                ...
                @if($procesos and count($procesos) > 0)
                    ...
                    @if($actividades and count($actividades) > 0)
                        ...
                        <div class="pt-4">
                            @if( !is_null($entradas) and count($entradas) > 0)
                                    ...
                                    @slot('body')
                                        @foreach($entradas as $entrada)
                                            <x-utils.tables.row>
                                                ...
                                                <x-utils.tables.body
                                                    class="text-xs">{{ $entrada->codigo }}</x-utils.tables.body>
                                                <x-utils.tables.body
                                                    class="text-xs">{{ $entrada->nombre }}</x-utils.tables.body>
                                            </x-utils.tables.row>
                                        @endforeach
                                    @endslot
                                </x-utils.tables.table>
                            ...
                            @else
                                <x-utils.message-no-items
                                    ...
                                    text="Al parecer ya asignó todas las entradas disponibles.">
                                    @slot('icon')
        ...
        <x-slot name="footer">
            <x-jet-button
                wire:click="asignarEntradas"
                wire:target="asignarEntradas"
                wire:loading.class="cursor-wait"
                wire:loading.attr="disabled">
                <x-icons.load wire:loading wire:target="asignarEntradas" class="h-5 w-5"/>
                Guardar
            </x-jet-button>
        </x-slot>
    </x-jet-dialog-modal>
