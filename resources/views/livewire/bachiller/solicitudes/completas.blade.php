                    ...
                    @slot('body')
                        @foreach($solicitudSeleccionado->documentos as $i => $doc)
                            <x-utils.tables.row>
                                ...
                                <x-utils.tables.body>
                                    <buttons
                                        class="inline-flex items-center text-{{ $doc->estado->color }}-700 bg-{{ $doc->estado->color }}-100 rounded-md text-xs px-3 py-1 whitespace-nowrap">
                                        {{ $doc->estado->nombre }}
                                    </buttons>
                                </x-utils.tables.body>
                                <x-utils.tables.body>
                                    <div
                                        ...
                                        <x-utils.buttons.danger title="Denegar requisito" wire:click="denegarDocumentoRequisito({{$doc->id}})">
                                            <x-icons.x class="icon-4 mr-1"/>
                                        </x-utils.buttons.danger>
                                        <x-utils.buttons.success title="Aprobar requisito" wire:click="aprobarDocumentoRequisito({{$doc->id}})">
