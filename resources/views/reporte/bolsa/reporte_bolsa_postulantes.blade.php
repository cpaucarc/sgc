    ...
    <table class="mb-0" style="width: 100%">
        ...
        <tr>
            <td style="width: 80%; text-align: left;">
                <h4 class="font-weight-bold">Bolsa de trabajo</h4>
            </td>
        ...
        @foreach($fac->escuelas as $esc)
            @if(count($esc->bolsaPostulante))
                ...
                <table class="table table-sm table-bordered">
                    ...
                    <tbody>
                    @foreach($esc->bolsaPostulante as $i => $bolsa)
                        <tr>
                            <td style="padding: 3px 6px">{{ $i + 1 }}</td>
                            <td style="padding: 3px 6px">{{ $bolsa->semestre->nombre }}</td>
                            <td style="padding: 3px 6px">{{ $bolsa->postulantes }}</td>
                            <td style="padding: 3px 6px">{{ $bolsa->beneficiados }}</td>
                            <td style="padding: 3px 6px">{{ $bolsa->escuela->nombre }}</td>
                            <td style="padding: 3px 6px">{{ $bolsa->fecha_inicio->format('d-m-Y') }}</td>
                            <td style="padding: 3px 6px">{{ $bolsa->fecha_fin->format('d-m-Y') }}</td>
                        </tr>
                    @endforeach
                    </tbody>
                </table>
            @else
                <p style="font-size: 0.8rem">No hay información sobre Bolsa de Trabajo del Programa
